...
namespace BarelyFunctional
{
    ...
    public class RaytraceInstancing : MonoBehaviour
    {
        ...
        void OnRenderImage(RenderTexture src, RenderTexture dest)
        {
            ...
            Matrix4x4 matrix1 = Matrix4x4.TRS(target.position, target.rotation, target.localScale);
            rayTracingAccelerationStructure.AddInstance(config, matrix1, null, (uint)0);
            Matrix4x4 matrix2 = Matrix4x4.TRS(target.position + target.forward * 2, target.rotation, target.localScale);
